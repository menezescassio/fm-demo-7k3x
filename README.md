...
- **S2, Result + install.** Each role's primary task lands on its own cited result on the account's own files (rent reviews for a broker, condo-fee gaps for a listing manager, renewal windows for legal), every figure linking the exact page or row. The close is the core mechanic: **"Make this my Monday morning brief"** turns the one-off task into a standing weekly job that returns without anyone asking.
